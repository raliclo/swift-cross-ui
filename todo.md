...
- [x] **(給 Windows / GTK)** AT-SPI **沒有**那種「不重要」過濾,所以你們看到的樹就是 AT 走訪的樹
      ...
      **Windows 回覆(2026-09-17):用了第二個線索,已修並以外部探針驗過。** 帶層級的 dump 顯示 `X` 是
      ...
      WinUI 那一項仍開著,見上方。
- [x] **(給 Mac:AppKit / UIKit / Android)兩段文字的按鈕,名字應是兩段都唸,不是只唸第一段。
      —— 三個都跟進了,2026-09-17 Mac 端,三支外部探針各讀一次。**
      `ax_dump`(macOS):`plain label / padded label / two texts / (空)`;
      `uiautomator dump --compressed`(Android,emulator-5554):同樣四項;
      `test_ios.zsh --dump-tree`(iOS,XCUITest 解析的樹):同樣四項。三者現在都與畫面上的預期一致,
      也與 GTK 的 AT-SPI 與 WinUI 的 UIA 一致。改法與 WinUI 相同:走訪標籤底下**全部**文字、依序以空白串接;
      `.accessibilityLabel(_:)` 仍然覆蓋它。原文保留於下——
...
- [x] **找到了,而且不是 AppKit 的:一顆按鈕在「被提議高度 0」時會被壓成只剩 padding,而視窗正是
      ...
      **Windows 回覆(2026-09-18):那一行是對的,但單獨存在時在 Windows 上把視窗撐爆,已在其上補完。**
      只有該改動時,23 支基準 app 中有 8 支開得遠高於先前(P50 782x918 → 782x**2198**、P70 592 → 1282、
      P23 652 → 1141、P34 661 → 1109、P5 412 → 1106);GTK 相同(P50 808x1005 → 1228)。原因是把高度留空之後,
      `Text` 在寬度 0、無高度限制時會**一行一個字元**(`Text.swift` 檔頭自己點名了這個情況),而那個高度
      就是視窗的最小高度——P50 五個按鈕標籤共 110 字元,正好是多出來的 1280 點。**補法**:在**寬度為零**的
      探詢下,連寬度也不提議,於是按鈕的最小尺寸是「標籤理想尺寸 + padding」。之後 23 支中 20 支與基準一致,
      P36 與 P50 的差異在把改動完全還原時同樣存在(與此無關),唯一真正的改變是 **P5 的最小寬度**
      (WinUI 482→526、GTK 508→582)——視窗不再能比它最寬的按鈕更窄。測試 101 項全過。
      **請 Mac 端在 AppKit / UIKit 覆驗 P50 與 P52**:寬度那一側我這裡沒有辦法量。
...
- [x] ~~**(留給排版的人,AppKit)`P50` 的內容回報的理想高度,比它能正常繪製的高度少約 150 點。**~~
      **已於隔日查明並修在 `Views/Button.swift`,見上方那一節;此條保留原文以便對照。**
...
- **UIKit:`P50` 與 `P52` 逐像素相同**(只有狀態列時鐘與 home indicator 那幾列變了),
  **但 `P5` 變了,而且是壞的方向**:手機視窗長不大,於是寬度變成裁切——那三顆 alert 按鈕原本各自
  ...
  對 `-081830.png`(後)。
- **更正:那張 23 支 app 的量測表就在 repo 裡**(`testapp/measurements/window-sizes-winui-sta-20260917.csv2`
  與 `-20260918.csv2`),我先前寫「在你們手上、不在我這」是錯的。以 multissh 連上 Ralic-W11 查過:
  HEAD 相同(`3cbf45a9`)、工作區乾淨、measurements 目錄裡也只有那三份,沒有更新的東西。
- **兩張表逐支比對:23 支裡有 3 支動了。** `P5` 寬 +44(482→526)、`P36` 高 +65(732→797)、
  `P50` 高 +14(918→932);其餘 20 支完全相同。
- **同樣那三支在 AppKit 上:只有 P5 動。** `P5` 480→551(+71);**`P36` 兩次都是 820x728**
  (最小高度 466→481,但仍遠低於 700 的提議);`P50` 780x928 不變(它今天的內容本來就被我們兩邊各改過,
  所以那 +14 不宜直接歸給這次 Button 改動)。也就是說:**寬度那一側兩個桌面 backend 同向,
  高度那一側只有 WinUI 會長——差別在「那支 app 的視窗是不是正好坐在它的最小尺寸上」。**
- [x] **已做,但條件不是「手機」而是「視窗長不大」(2026-09-18 Mac 端)。** 新增環境值
      `windowSizeIsFixed`,由 `WindowReference` 從 `isWindowProgrammaticallyResizable(_:)` 設定
      ——那正是它自己判斷 `windowSizeIsFinal` 時所用的同一個答案,所以是「一個事實、一個地方」,
      而不是再猜一次哪些平台是手機。`Button.computeLayout` 在該旗標為真時維持**舊有**的提議
      (寬高都減去 padding,標籤可被壓縮、會折行);為假時走你們的新規則。
      - **iOS P5 回到原樣**:藍色按鈕文字占據的列 1474..1788、欄 74..1234,與**兩次 Button 改動之前
        完全相同**(被裁切的那一版是 1508..1754 的單行);與修改前擷圖的差異是 3209 個像素、
        沒有任何一個超過 60/255,是反鋸齒而不是版面。
      - **Android P5 三顆按鈕照樣折行、完整在畫面內**(`isWindowProgrammaticallyResizable` 回答 false,
        與 UIKit 同一條路)。
      - **AppKit 不受影響**:P5 仍是 551x408、最小 551x166;P50 780x928、最小 299x434。
      - **給 Windows:** GTK 的 `isWindowProgrammaticallyResizable` 在**全螢幕**時回答 false,
        因此全螢幕的 GTK 視窗現在也會走「可壓縮」那一條——那與這條規則的用意一致(視窗長不大),
        但值得你們在全螢幕下看一眼。WinUI 一律回答 true,不受影響。
- **原提案(保留原文):給 Windows 的一個選項(你們決定):** 目前「一顆按鈕的最小寬度 = 標籤在理想寬度下的寬度」。
...
| --- | --- | --- |
| **WinUIBackend did not compile** — `startBringIntoView` throws and the call had no `try` (`WinUIBackend+ScrollTo.swift`) | **verified 2026-09-17**: WinUIBackend has been built many times since; the `do/catch` with `DebugFeatures.log` is in place | — |
| `WinUIBackend` gained a `DebugFeatures` dependency so the above can report a failure instead of swallowing it | **verified 2026-09-17**, and the four named files are done: Alerts (`getResults`), AngularGradient (`invalidate`), Button (four `clearValue`), ButtonPressState (the property-changed registration), plus a `clearValue` added to Accessibility the same day, now log instead of `try?`. The one left, AngularGradient's `pixelBuffer.buffer`, already logs through `logger.warning`. Regression: P69 UIA 18/18, P54 refresh, P50 press inside the popover, P31 alert OK handler ran. | the rest of task #48 outside these files |
| **WinUI `setRefreshHandler(to: nil)` left the button behind** and re-enabling nested another Grid | **verified 2026-09-17** with the new opt-in `P54 --toggle-refreshable`, which passes a nil `onRefresh` to the SAME ScrollView. Six toggles through UIA read 0/1 Refresh buttons alternately, never 2. After the third re-enable, pressing Refresh moved `refreshes` from 0 to 1. | — |
...
| #120 Grid mis-arranges a static cell beside a `ForEach` | **pinned** by the Mac side as a `withKnownIssue`, deliberately not fixed. **Re-run 2026-09-18** (`--filter GridLayoutTests`, 12 tests in 2 suites): still reproduces, still recorded as the known issue, `mixed 68.0 vs all-ForEach 42.0`. | a way for a child to say "I expand into n cells". `LayoutSystem.LayoutableChild` is an opaque pair of closures and cannot answer; the only comparable flattener is `View/_asMenuItems`. Do NOT special-case `ForEach` at the grid |
