...
- **P7 on GTK, now with content sizes:** `total=420.0 minLeading=31.0 minTrailing=36.0 -> bounds min=31 max=384 currentSidebar=200 leadingContent=200.0x140.0 trailingContent=207.0x77.0`, three identical lines. The sidebar figure of 200/420 that settled #556 is confirmed at the layout level. Two things the earlier verdict could not see: the trailing child answers 207 to an offer of 220, and the two children have very different heights (140 against 77). Neither is a pane-ratio mismatch, so #556's verdict stands, but both are worth a look before that entry is closed.
