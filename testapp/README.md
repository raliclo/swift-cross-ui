...
| Question | File |
| --- | --- |
| Where do I run this issue, and does the answer count? | `platform-test-plan.md` |
| What are the steps for app PN? | `UI-test-plan_en.md` |
| What is the plan for the GtkBackend work? | `linux-test-plan.md` |
| What is the plan for the AppKit/Android/iOS work? | `bug-test-plan.md` |
| What is the state of every upstream issue? | `issues.csv` |
| Which commit fixes what, and can it be submitted? | `issue_commits.csv` |

`platform-test-plan.md` is the entry point: it maps all 40 covered issues onto
...
split-view behaviour. `platform-test-plan.md` has the full mapping.
...
`UI-test-plan.md`, the Traditional Chinese half of the test plan, and
`UI-test-results.md`. Edits to the test steps belong in both language files even
though only `UI-test-plan_en.md` is committed.
