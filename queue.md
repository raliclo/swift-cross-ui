...
- [~] **15. #28 Animation — 時鐘已落地並驅動過,動畫引擎未做** — 缺的那一半是 backend requirement:`CADisplayLink`/`CVDisplayLink`/`Timer.scheduled` 在整個 `Sources/` 是 0 命中,五個 backend 都沒有逐幀訊號。新增 `BackendFeatures.FrameClocks`(app 層級,因為只有 GTK 的是逐 widget)。**P64 直接驅動它並量到速率**:macOS 60.0 Hz、間隔中位數 16.7ms;iOS 59.7 Hz、中位數 16.7ms(模擬器實跑,不只編譯)。AppKit/UIKit 已編譯並執行;Android/GTK/WinUI 已寫,各自標明未編譯與要查什麼。**剩下的是框架端**:`Animation` 型別、`withAnimation`、可插值的值——那是另一個決定,不是這一個
